...
def already_at_mission_goal(start, goal, tolerance=1e-9) -> bool:
    """Recognize the terminal zero-length case without constructing a Polyline."""
    start = np.asarray(start, dtype=float).reshape(3)
    goal = np.asarray(goal, dtype=float).reshape(3)
    return bool(np.all(np.isfinite(start)) and np.all(np.isfinite(goal))
